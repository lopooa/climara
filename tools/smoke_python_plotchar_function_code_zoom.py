...
def main():
    ...
    assert [event.kind for event in parsed.events] == ["text", "xzoom", "text"]
    assert parsed.events[1].value == 200
    ...
    assert parsed.events[1].kind == "yzoom"
    assert parsed.events[1].value == 200
    ...
    assert parsed.events[1].kind == "zzoom"
    assert parsed.events[1].value == 200
    assert parsed.events[1].use_q_unit is True

    plain = compute_real_string(f"~A~{a}{b}")
    xzoom = compute_real_string(f"~A~{a}~X200~{b}")
    yzoom = compute_real_string(f"~A~{a}~Y200~{b}")
    yzoom_q = compute_real_string(f"~A~{a}~Y200Q~{b}")
    zzoom = compute_real_string(f"~A~{a}~Z200~{b}")
    zzoom_q = compute_real_string(f"~A~{a}~Z200Q~{b}")
    xzero = compute_real_string(f"~A~{a}~X0~{b}")

    assert xzoom.text == a + b
    assert yzoom.text == a + b
    assert zzoom.text == a + b

    assert xzoom.metrics.width > plain.metrics.width
    assert yzoom.metrics.height > plain.metrics.height
    assert zzoom.metrics.width > plain.metrics.width
    assert zzoom.metrics.height > plain.metrics.height

    assert yzoom_q.metrics.height > 0.0
    assert zzoom_q.metrics.height > 0.0
    assert yzoom_q.state.pcgetr("YE") != yzoom.state.pcgetr("YE")
    assert zzoom_q.state.pcgetr("YE") != zzoom.state.pcgetr("YE")

    assert xzero.metrics.width > 0.0
    assert abs(xzero.metrics.width - plain.metrics.width) < plain.metrics.width

    print("✅ Python Plotchar X/Y/Z zoom function-code smoke passed")
