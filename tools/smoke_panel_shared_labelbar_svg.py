...
def main() -> None:
    ...
    out_base = Path("outputs") / "figures" / "panel_shared_labelbar_svg_smoke"
    ...
    if missing:
        raise RuntimeError(f"shared panel labelbar SVG missing: {missing}")

    labelbar = panel.resources.get("labelbar")
    if labelbar is None:
        raise RuntimeError("panel.resources does not contain shared labelbar.")

    print(f"✅ shared panel labelbar smoke passed: {output}")
