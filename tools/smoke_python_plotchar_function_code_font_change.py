...
def compatible_font_and_chars(min_count: int) -> tuple[int, str, bool]:
    ...
    raise SystemExit(
        f"SKIP: font{DEFAULT_FONT} does not expose enough uppercase/digit glyphs "
        "for this smoke."
    )
...
def main():
    ...
    plain = compute_real_string(f"~A~{body1}{body2}")
    ...
    assert reset.metrics.width > switched.metrics.width
    ...
    if used_secondary:
        suffix = f"font{DEFAULT_FONT} -> font{target_font}"
    else:
        suffix = f"font{DEFAULT_FONT} fallback target; no compatible secondary fontcap found"

    print(f"✅ Python Plotchar function-code font-change smoke passed ({suffix})")
