...
def main() -> None:
    ...
    out_base = Path("outputs") / "figures" / "contour_panel_svg_smoke"
    ...
    cgr.gsn_text_ndc(
        wks,
        "gsn_csm_contour_map -> gsn_panel -> SVG",
        0.50,
        0.95,
        {
            "txFontHeightF": 0.027,
    ...
    if missing:
        raise RuntimeError(f"Contour panel SVG missing required content: {missing}")

    print(f"✅ contour panel SVG smoke passed: {output}")
